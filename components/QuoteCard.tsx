...
export const QuoteCard = forwardRef<HTMLDivElement, QuoteCardProps>(({ quote, isLoading }, ref) => {
  // Clean content to prevent double quoting in UI
      ...
      <div className="relative z-10 flex-grow flex flex-col h-full">
        {isLoading ? (
            ...
            <div key={quote.content} className="flex flex-col h-full p-8 md:p-12 animate-gentle-fade">
                ...
                <div className="flex-grow flex flex-col items-center justify-center relative w-full my-6">
                  <div className="relative px-2">
                      {/* Quote Symbol Top - Adjusted spacing */}
                      <div className="absolute -top-12 -left-4 md:-left-8 text-8xl text-stone-200 font-serif leading-none select-none font-black opacity-60">“</div>

                      {/* Text Content - Typography Optimized */}
                      <h1 className={`
                        font-serif font-bold text-gray-900 text-center z-10 relative px-2
                        ${isLongText ? 'text-2xl md:text-3xl leading-loose tracking-wide' : 'text-3xl md:text-4xl leading-relaxed tracking-widest'}
                      `}>
                        {cleanContent}
                      </h1>

                      {/* Quote Symbol Bottom */}
                      <div className="absolute -bottom-12 -right-4 md:-right-6 text-8xl text-stone-200 font-serif leading-none select-none font-black rotate-180 opacity-60">“</div>
                  </div>
                </div>

                {/* Footer */}
                <div className="mt-auto pt-4 relative z-20">
                  {/* Author, Source, and Year removed as requested */}
                  
                  {/* Enhanced Separator Line */}
                  <div className="relative pt-6 border-t border-mao-red/10">
                     ...
                     {/* Explanation Text */}
                     <p className="text-sm text-stone-700 font-serif leading-7 text-justify tracking-wide px-1 mb-4 font-medium opacity-90">
                      {quote.explanation}
                    </p>
                    
                    {/* Date Stamp */}
                    <div className="w-full flex justify-end border-t border-dashed border-stone-200 pt-3">
                      <span className="text-[11px] text-stone-400 font-serif tracking-widest font-medium">{todayDate}</span>
                    </div>
                  </div>
                </div>
            </div>
